...
public class InviteesAdapter  extends RecyclerView.Adapter<InviteesAdapter.ItemViewHolder> implements Filterable {

    private List<OrderInvitees> mDataList;
    private List<OrderInvitees> mDataListAll;
    String profilePath; //프로필 Uri 경로


    //constructor
    public InviteesAdapter(List<OrderInvitees> items) {
        mDataList = items;
    ...
    //data set changed
    public void dataSetChanged(List<OrderInvitees> exampleList) {
    ...
    public ItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.invitees_item,
                parent, false);
    ...
    public void onBindViewHolder(@NonNull final ItemViewHolder holder, final int position) {
        OrderInvitees currentItem = mDataList.get(position);
    ...
    private Filter exampleFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            List<OrderInvitees> filteredList = new ArrayList<>();

            if (constraint == null || constraint.length() == 0) {
                filteredList.addAll(mDataListAll);
            } else {
                String filterPattern = constraint.toString().toLowerCase().trim();
                for (OrderInvitees item : mDataListAll) {
